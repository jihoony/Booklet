...
*   `-foliosize`: 한 시그니처(묶음)당 시트 수 (multifolio=on일 때 사용, 기본값: 8)
...
입력 파일이 매우 큰 경우(예: 200페이지), 프로그램이 자동으로 시그니처 모드를 활성화하여 8장(32~64페이지)씩 묶어서 출력합니다. 사용자는 출력된 묶음들을 각각 접어서 합치면 됩니다.
